...
{/if}
<img{if !$RVAS_PENDING} src="{$current.selected_derivative->get_url()}" {if isset($rvas_display_size)}width={$rvas_display_size[0]} height={$rvas_display_size[1]} data-natural-w={$rvas_natural_size[0]}{else}{$current.selected_derivative->get_size_htm()}{/if}{/if} alt="{$ALT_IMG}" id="theMainImage"{if !isset($rvas_display_size)} usemap="#map{$current.selected_derivative->get_type()}"{/if} title="{if isset($COMMENT_IMG)}{$COMMENT_IMG|@strip_tags:false|@replace:'"':' '}{else}{$current.TITLE_ESC} - {$ALT_IMG}{/if}" itemprop=contentURL>
